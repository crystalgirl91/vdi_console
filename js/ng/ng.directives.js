	...
	.factory('localize', ['$http', '$rootScope', '$window', function($http, $rootScope, $window){
		var localize = {
			...
			getLangUrl: function(lang) {
				...
				var aa = $Domain + "/thor/toolkit/lang_" + lang.langCode + ".js";
...
angular.module('app.navigation', [])
	...
	.controller('NavGroupController', ['$scope', function($scope) {
	    ...
	    $scope.user_power = [];
	    var list = localStorage["power"] ? localStorage["power"].split(",") : [];
    	list.forEach(function(item){
	    	$$$power_lists.forEach(function(p){
	    		if(item === p.key){
	    			$scope.user_power.push(p.value);
	    		} 
	    	}) 
	    })
	    $scope.flag = false;
	    $scope.user_power.forEach(function(item){
	    	if(item === $scope.title){
	    		$scope.flag = true;
	    	}
	    })
	...
	.directive('navGroup', function() {
		return {
			...
			template: '\
				<li data-ng-show="flag" data-ng-class="{active: active, versionHide: langVersion && langVersion == \'cloudPC\'}">\
					<a href="">\
						<i data-ng-if="hasIcon" class="{{ icon }}"><em data-ng-if="hasIconCaption"> {{ iconCaption }} </em></i>\
						<span class="menu-item-parent" data-localize="{{ title }}"></span>\
					</a>\
	...
	.directive('navItem', ['ribbon', '$window', function(ribbon, $window) {
		return {
			...
			link: function(scope, element, attrs, parentCtrls) {
				...
				scope.$watch('active', function(newVal, oldVal) {
					if (newVal) {
						...
						$window.document.title = $$$I18N.get(scope.title);
						scope.setBreadcrumb();
					} else {
			...
			replace: true,
			template: '\
				<li data-ng-show="flag" data-ng-class="{active: isActive(view)}">\
...
angular.module('app.smartui', [])
	...
	.directive("uiWebUpload", ["$http", "$rootScope", function($http, $rootScope){
		// var uploadURL = $Domain + "/thor/isos";
		...
		return {
			...
			controller: ["$scope", function(scope){
				scope.$on("progress", function(e, size){
					if(size.id === scope.upload_id){
						...
						scope.progressPercent = (size.loaded / size.totalSize * 100).toFixed(2) + "%";
			...
			link: function(scope, element, attrs){
				...
				var maxSize,filetype,url;
				var form = element.find("form");
				form.attr("action", url);
				function checkConfig(file,config){
					var filetype = attrs.uiUploadType;
					var maxSize = Number(attrs.uiUploadLimit.substring(0,attrs.uiUploadLimit.indexOf("G"))) || 10;
					...
					if(filetype && file.name.split(".").reverse()[0].toUpperCase() !== filetype.toUpperCase()){
						$.bigBox({
							title : "Error",
				...
				element.find("input[type=file]").on("change", function(e){
					...
					if(isValid){
						...
						$http.post($Domain + attrs.uiUploadUrl, data, {
							...
							finish();
							success();
							$rootScope.$broadcast("finishUpload", true)
						},function(){
							clearDOM();
							finish();
						});
