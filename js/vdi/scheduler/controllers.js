...
.controller("vdiSchedulerViewController", ["$scope", "Scheduler", "Scene", "SchoolRoom", "$modal", function($scope, scheduler, Scene, schoolroom, $modal){
	...
	var fn_update_config = function(config){
		...
		var week_start = 0;
		$scope.weeks.splice(0, $scope.weeks.length);
		...
		var dialog = $modal.open({
			...
			controller: function($scope, $modalInstance){
				...
				$scope.formValid = function(){
					return $scope.$$childTail.schedulerForm.$valid;
				};
				$scope.ok = function(){
					var data = {
